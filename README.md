...
3. 点击悬浮球时丢弃上下行包约 **450ms ± 80ms**，并重置会话  
...
| 版本 | 说明 |
|------|------|
| 0.1.7 | 延迟颜色改为绿/黄/红；未知显示 999ms |
| 0.1.6 | 悬浮球深色底 + 铜色描边，高延迟数字更易读 |
| 0.1.5 | 启动时：游戏已运行则切前台，未运行则冷启动 |
| 0.1.4 | Wi‑Fi 拔线更快生效（即时丢包 + RST）；4.5s 锁定防连点风暴 |
| 0.1.3 | 减少无故重连：健康检查防抖、不误杀空闲长连接、重建默认只重启转发不拆 VPN 壳 |
| 0.1.2 | 加固用户态转发线程模型，修复多次拔线后「状态开启但炉石全离线」 |
| 0.1.1 | 覆盖安装/异常退出后的死隧道清理与状态对账 |
| 0.1.0 | 首发：常驻隧道 + 悬浮球拔线 + 延迟显示 |
